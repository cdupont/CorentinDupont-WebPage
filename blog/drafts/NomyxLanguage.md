...
Discover the Nomyx Language

The Nomyx language is articulated around one type: Nomex.

Let's go through the examples contained in the file Examples.hs.
...
The type of this function is Rule, which is a synonym for `Nomex ()`: `type Rule = Nomex ()`.
A Nomex is an instance of Monad.
As its name hints, this function does nothing.
If you submit it, however, it will trigger an event "Rule Proposed".
This event will wake up the handlers of the active rules that are subscribed on this event.
Thus, event if this rule `nothing` doesn't do anything, submitting it can trigger a lot of things: that usually includes a votation.
When you submit it, the votation rule (usually rule #1) is wake up and will display the forms for voting.
If the vote comes out as positive, rule #1 will activate and effectively run the incoming rule.

Only a rule can activate a proposed rule: that's what make the beauty of the game.
...
The output system is not merely a logging system but is dynamic.
That is to says that an output can be automatically updated by the system.
...
If new players are coming or players are leaving, the message would update automatically.
...
`NomexNE` is a sub-DSL of `Nomex`, collecting all instructions that doesn't have any effect.
By comparison, `Nomex` collects all instructions that can have an effect.
Of course, a rule can have an effect (but doesn't return anything), that's why it is typed `Nomex ()`.
...
Our next rule will be:

    createBankAccount :: Rule
    createBankAccount = void $ newVar_ "account" (0::Int)

We are creating a variable with the name "account", the type "Int" and the initial value O.
All the variable related functions are located in the module Language.Nomyx.Variables.
You can see this variable appear in the section "Details" at the bottom of the scree.
This variable can now be accessed by any other rules.
In the example.hs file you can see how to display this variable (using dynamic outputs) and modify it (win money, transfer money...).
...
Once he validate his entry, the poem will be displayed on the screen of each player.
...
The callback takes the string entered by the player as an input, and can perform any effect on the game (thus the `Nomex` type).

There are similar functions for creating text boxes, buttons, check boxes and radio buttons.
...
We thus need to use the function `liftEffect`:
...
So, what if we want to display the current time instead of the activation time?
...
Here, instead of displaying a simple string, we are registering a small program take will yield the current time each time it is called.
...
In the section on Inputs, we showed how to create single fields such as a text field, and how attach a callback to them.
But what if I want several input fields combined together?
What If I want to include other events happening when a player is arriving or a specific time is reached?
This is done throw the event system of Nomyx.

The inputs (text box, text area, button, checkboxes and radios) are in fact standard events in Nomyx.
...
- a message sent by another rule,
...
You could then express nice combinations of events.
For example, if you need to file a record with two fields containing the name and surname of your player, you can do everything in one go:
...
Two input fields will appear on the screen of player number 1, and their results will be combined into the NameSurname record.
...
If some inputs are found, they will be automatically displayed to the players, in the "Inputs/Outputs" zone of the screen.
We are then merging in the same representation two different activities that GUIs are traditionaly performing separately:
- displaying some forms,
- attaching the logic to the result of the forms. 
Here everything is performed in the same language.

The players doesn't control where the forms are displayed and how they are styled: the objective of this DSL is to let the players concentrate on the logic of the rules.
Indeed the rules have to be read by other players and voted on, so they should keep simple.
...
This is specially useful for voting systems for examples, where each player has to complete a vote form, and all the results are collected to form a single "vote" event.
...
This is performed by the function `listNomexNE`, as showed in the example function `moneyTransfer`.
...
If that's the case, it will display successively two fields, one for asking the player you would like to transfer funds to, and the second for the amount.
...
Wow! We are nearly done.
With all that we learned so far, we are able to construct more interresting rules.
Let's now change our political system: it's the revolution!
We want to go from the current democratie to a monarchy.
...
So, this rule first suppresses the rule number 1, which is usuelly the democratie.
Then, it installs a king, using a player number.
Here `king` is just a variable keeping the player number of the King, so that other rules can refer to it.
Then, `revolution` adds directly another rule in the pending rules, using `addRuleParams`.
We pass the title of the added rule, the code (using both a function name and a string to display it) and a description.
...
This type of rules such as `revolution` is really what makes a Nomic game: the ability for a rule to manipulate other rules, including itself.


# Victory

The second most important feature of a Nomyx game is to be able to change the way how to win the game.
...
Similarly to the output system, the victory is dynamic: it's a small program `NomexNE [PlayerNumber]` that returns the list of victorious people.
...
You could also be bold an propose directly your own victory:
